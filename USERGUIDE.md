...
let serviceClient = new Client(baseUrl, privateKey);
```
## Text messages
...
```Node
let outMessage = serviceClient.GetOutMessageAsync(transactionId);
...
```Node
serviceClient.deleteOutMessageAsync(transactionId);
```
...
await serviceClient.createOutMessageBatchAsync([outMessage1, outMessage2]);
```
...
serviceClient.createStrexTransactionAsync(transaction);
```
...
```Node
let reversalTransactionId = serviceClient.reverseStrexTransactionAsync(transactionId);
```
...
```C#
let transaction = serviceClient.getStrexTransactionAsync(transactionId);
...
serviceClient.saveOneClickConfigAsync(config);
```
...
serviceClient.createStrexTransactionAsync(transaction);
transaction = serviceClient.getStrexTransactionAsync(transactionId);
...
```Node
let lookup = serviceClient.getLookupAsync("+4798079008");
...
let keywordId = serviceClient.createKeywordAsync(keyword);
```
...
#### Request
```
boolean success = await serviceClient.verifyPinCodeAsync(pincode.transactionId, pin);
```
